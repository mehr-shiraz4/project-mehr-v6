...
// auth.js — احراز هویت Supabase Auth  v6.3
...
function _codeToEmail(code) {
  const clean = code.trim()
    ...
    .replace(/\D/g, '');
  return `p${clean}@mehr4shiraz.com`;
}
...
async function registerRequest() {
  ...
  try {
    ...
    // ✅ روش بدون ارسال ایمیل:
    // ۱) ورود anonymous (هیچ ایمیلی نمی‌فرسته)
    // ۲) سپس ایمیل و پسورد رو به حساب وصل می‌کنیم
    const { data: anonData, error: anonErr } = await _supabase.auth.signInAnonymously();
    if (anonErr) throw anonErr;

    const { data: updateData, error: updateErr } = await _supabase.auth.updateUser({ email, password });
    if (updateErr) {
      await _supabase.auth.signOut();
      throw updateErr;
    }
