...
document.addEventListener('DOMContentLoaded', function() {
  ...
  /* ===== LOGIN ROLE / LEVEL (مای‌مدیو) ===== */
  document.querySelectorAll('[data-login-role]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      selectLoginRole(this.getAttribute('data-login-role'));
    });
  });
  document.querySelectorAll('#regLevelGrid .level-chip').forEach(function(btn) {
    btn.addEventListener('click', function() {
      document.querySelectorAll('#regLevelGrid .level-chip').forEach(function(b) { b.classList.remove('active'); });
      this.classList.add('active');
      var lv = document.getElementById('regLevel');
      if (lv) lv.value = this.getAttribute('data-level');
    });
  });
  document.querySelectorAll('[data-active-level]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      setActiveLevel(this.getAttribute('data-active-level'));
    });
  });
  document.querySelectorAll('[data-cartable-tab]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      document.querySelectorAll('[data-cartable-tab]').forEach(function(b) { b.classList.remove('active'); });
      this.classList.add('active');
      var tab = document.getElementById('cartableTab');
      if (tab) tab.value = this.getAttribute('data-cartable-tab');
      if (typeof renderObserverCartable === 'function') renderObserverCartable();
    });
  });
  var sendToObserverBtn = document.getElementById('sendToObserverBtn');
  if (sendToObserverBtn) sendToObserverBtn.addEventListener('click', submitReportToObserver);
  var sendToObserverBtn2 = document.getElementById('sendToObserverBtn2');
  if (sendToObserverBtn2) sendToObserverBtn2.addEventListener('click', submitReportToObserver);
  ...
  var drawerMap = {
    ...
    'drawer-office':    function() { drawerShow('officeSection'); },
  ...
  if (filterDomain) filterDomain.addEventListener('change', function() {
    setActivityFilter('domain', this.value);
  });
  var filterPhase = document.getElementById('filterPhase');
  if (filterPhase) filterPhase.addEventListener('change', function() {
    setActivityFilter('phase', this.value);
  });
  var filterLevel = document.getElementById('filterLevel');
  if (filterLevel) filterLevel.addEventListener('change', function() {
    setActivityFilter('level', this.value);
  });
  var filterSearch = document.getElementById('filterSearch');
  if (filterSearch) filterSearch.addEventListener('input', function() {
    setActivityFilter('q', this.value);
  });
  var filterParvareshiBtn = document.getElementById('filterParvareshiBtn');
  if (filterParvareshiBtn) filterParvareshiBtn.addEventListener('click', function() {
    activityFilter = { domain: 'پرورشی', phase: 'اول‌مهر', level: 'all', q: '' };
    if (filterDomain) filterDomain.value = 'پرورشی';
    if (filterPhase) filterPhase.value = 'اول‌مهر';
    if (filterLevel) filterLevel.value = 'all';
    if (filterSearch) filterSearch.value = '';
    renderActivities();
  });
  var filterMehrBtn = document.getElementById('filterMehrBtn');
  if (filterMehrBtn) filterMehrBtn.addEventListener('click', function() {
    activityFilter = { domain: 'all', phase: 'اول‌مهر', level: 'all', q: '' };
  ...
  var exportParvareshiBtn = document.getElementById('exportParvareshiBtn');
  if (exportParvareshiBtn) exportParvareshiBtn.addEventListener('click', exportParvareshiList);

  var openGuideBtn = document.getElementById('openGuideBtn');
  if (openGuideBtn) openGuideBtn.addEventListener('click', function() {
    var m = document.getElementById('guideModal');
    if (m) { m.style.display = 'flex'; }
  });
  var closeGuideBtn = document.getElementById('closeGuideBtn');
  if (closeGuideBtn) closeGuideBtn.addEventListener('click', function() {
    var m = document.getElementById('guideModal');
    if (m) m.style.display = 'none';
  });
  var guideModal = document.getElementById('guideModal');
  if (guideModal) guideModal.addEventListener('click', function(e) {
    if (e.target === guideModal) guideModal.style.display = 'none';
  });

  /* ===== CARTABLE FILTER ===== */
  var cartableFilter = document.getElementById('cartableFilter');
  if (cartableFilter) cartableFilter.addEventListener('change', function() {
    if (typeof renderObserverCartable === 'function') renderObserverCartable();
  });
...
function buildReportText() {
  ...
  return {
    ...
    sent_to_observer: true
  };
}

async function submitReportToObserver() {
  const r = buildReportText(); if (!r) return;
  const btn = document.getElementById('sendToObserverBtn');
  const btn2 = document.getElementById('sendToObserverBtn2');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ در حال ارسال...'; }
  if (btn2) btn2.disabled = true;
  try {
    const result = await sbReports.add(r);
    const created = Array.isArray(result) ? result[0] : result;
    reports.unshift(created || { ...r, id: Date.now() });
    renderReportsList();
    if (typeof renderObserverCartable === 'function') renderObserverCartable();
    if (typeof updateDashboard === 'function') updateDashboard();
    toastOk('گزارش به کارتابل ناظر ارسال شد');
    ['rptDone','rptIssues'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  } catch(e) {
    // fallback محلی تا ناظر در همین نشست ببیند
    const local = { ...r, id: 'local_' + Date.now() };
    reports.unshift(local);
    renderReportsList();
    if (typeof renderObserverCartable === 'function') renderObserverCartable();
    toastErr('ذخیره سرور ناموفق بود؛ گزارش موقتاً محلی ثبت شد. خطا: ' + e.message);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '🗂️ ارسال به کارتابل ناظر'; }
...
<style>
  ...
  tr:nth-child(odd) { background: #E8F5EF; }
...
function renderReportsList() {
  ...
  if (user && !['admin','inspector'].includes(user.role)) {
    list = list.filter(r => !r.uploaded_by || String(r.uploaded_by) === String(user.id));
  }
  if (!list.length) {
    container.innerHTML = '<div class="empty-state"><div class="empty-ico">📭</div><div class="empty-text">هنوز گزارشی ثبت نشده است.</div></div>';
    return;
  }
  container.innerHTML = list.map(r => `
    <div class="info-card" style="margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px;">
        ...
        <span>${reviewStatusLabel(r.review_status || 'pending')}</span>
      </div>
      <p style="font-size:13px;margin:4px 0;">👤 ${escapeHtml(r.name || '')} &nbsp;|&nbsp; 🏫 ${escapeHtml(r.school_name || '—')} &nbsp;|&nbsp; 🎓 ${escapeHtml(r.school_level || '—')}</p>
      <p style="font-size:12px;color:#666;">🗓 ${escapeHtml(r.period || '')} &nbsp;|&nbsp; 📊 ${escapeHtml(r.progress || '')} &nbsp;|&nbsp; 📅 ${escapeHtml(r.date || '')}</p>
      <p style="font-size:12px;color:var(--primary);margin-top:4px;">✓ ارسال‌شده به کارتابل ناظر</p>
