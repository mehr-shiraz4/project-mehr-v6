...
async function login() {
  ...
  try {
    ...
    // خواندن پروفایل از جدول (منبع حقیقت). با RLS درست، همیشه پروفایل خودِ کاربر خوانده می‌شود.
    let profile = null;
    try {
      const { data: profRows } = await _supabase
        .from('profiles').select('*').eq('id', authUser.id).limit(1);
      if (profRows && profRows.length) profile = profRows[0];
    } catch (e) {
      console.warn('Profile read error:', e.message);
    }

    // اگر پروفایلی پیدا نشد، یعنی ثبت‌نام ناقص بوده یا RLS مشکل دارد
    if (!profile) {
      await _supabase.auth.signOut();
      toastErr('پروفایل کاربری یافت نشد. لطفاً با ناظر منطقه تماس بگیرید.');
      return;
    }

    // اگر هنوز تأیید نشده، اجازه ورود نده
...
async function restoreSession() {
  try {
    ...
    // منبع حقیقت = جدول. اگر پروفایل نبود یا تأیید نشده، session را پاک کن.
...
async function _loadAllDataAndShowDashboard() {
  ...
  if (window.innerWidth <= 768) {
    ...
    // display را دستی ست نمی‌کنیم؛ نمایش منو فقط با کلاس open و CSS کنترل می‌شود
    document.getElementById('moreDrawer').classList.remove('open');
  }
...
/* ===== INSPECTOR PANEL ===== */
// دسترسی فقط بر اساس نقش کاربرِ احرازشده. رمز سمت‌کلاینت حذف شد چون
// قابل دور زدن بود؛ کنترل واقعی با RLS سمت سرور انجام می‌شود.
function openInspectorPanel() {
  ...
  if (_currentUser && ['inspector','admin'].includes(_currentUser.role)) {
    ...
    toastErr('شما اجازه دسترسی به پنل ناظر را ندارید.');
  }
}
