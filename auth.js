...
async function _loadAllDataAndShowDashboard() {
  const safe = (fn) => (typeof fn === 'function' ? fn() : Promise.resolve());
  await Promise.all([
    safe(loadUsers), safe(loadMembers), safe(loadActivities),
    safe(loadDocuments), safe(loadMeetings), safe(loadSchools),
    safe(loadObserverReports), safe(loadReports)
  ]);
  await safe(loadGroupManagers);
